...
# opt <- list(path = "/net/mulan/disk2/yasheng/simulation/SLM/data/",
#             block = 2,
#             herit = 0.5,
#             p = 0.01,
#             prop = 0.1,
...
snp_info_str <- "/net/mulan/disk2/yasheng/plink_file/simulation/sim"
...
##
snp_str <- paste0(opt$path, "snp_block", opt$block, "_her", opt$herit, "_cross", opt$cross, 
                  "_dist", opt$dist, "_ps", opt$p, ".txt")
geno_str <- paste0(opt$path, "geno_block", opt$block, "_her", opt$herit, "_cross", opt$cross, 
                   "_dist", opt$dist, "_ps", opt$p)
eff_str <- paste0(opt$path, "eff_block", opt$block, "_her", opt$herit, "_cross", opt$cross, 
                  "_dist", opt$dist, "_ps", opt$p,".txt")
pheno_str <- paste0(opt$path, "pheno_block", opt$block, "_her", opt$herit, "_cross", opt$cross, 
                    "_dist", opt$dist, "_ps", opt$p)
...
sel_cmd <- paste0("plink-1.9 --bfile ", snp_info_str, " --extract ", snp_str, 
                  " --make-bed --out ", geno_str)
system(sel_cmd)
...
set.seed(seed_str[as.numeric(opt$cross)])
if(opt$dist == "1"){
  eff_fix <- rnorm(length(snp_sel_fix_idx), 0, sqrt(as.numeric(opt$herit) / length(snp_sel_fix_idx)))
}
if(opt$dist == "2"){
  eff_fix <- rt(length(snp_sel_fix_idx), 4) / sqrt(length(snp_sel_fix_idx)) * as.numeric(opt$herit)
}
if(opt$dist == "3"){
  eff_fix <- rlaplace(length(snp_sel_fix_idx), 0, sqrt(as.numeric(opt$herit)/(2*length(snp_sel_fix_idx))))
}
